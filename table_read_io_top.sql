...
--   relid,
  ...
  (select (idx_scan / 1e6::int8) from pg_stat_user_tables where relid = pg_statio_user_tables.relid) idx_scan_mio,
  pg_size_pretty(bs * heap_blks_read) as heap_blks_read,
  case when heap_blks_hit + heap_blks_read > 0 then (100.0::numeric * heap_blks_hit / (heap_blks_hit + heap_blks_read))::int else null end heap_blks_hit_pct,
  pg_size_pretty(bs * idx_blks_read) as idx_blks_read,
  case when idx_blks_hit + idx_blks_read > 0 then (100.0::numeric * idx_blks_hit / (idx_blks_hit + idx_blks_read))::int else null end idx_blks_hit_pct,
  pg_size_pretty(bs * tidx_blks_read) as tidx_blks_read,
  case when tidx_blks_hit + tidx_blks_read > 0 then (100.0::numeric * tidx_blks_hit / (tidx_blks_hit + tidx_blks_read))::int else null end tidx_blks_hit_pct
FROM
