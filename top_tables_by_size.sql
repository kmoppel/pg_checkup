select
  table_name, is_part, total_size, table_size, toast_size, seq_scan, tx_freeze_age_m, last_seq_scan, tup_ins_m, tup_upd_m, tup_del_m, live_tup_m, dead_tup_pct
from (
with recursive /* pgwatch_generated */
    ...
    q_tstats as (
        select relid,
               ...
               date_trunc('hour', now() - last_seq_scan) last_seq_scan,
               n_tup_ins,
               n_tup_upd,
               n_tup_del,
               n_tup_hot_upd,
...
select
    ...
    -- n_dead_tup
    (100::numeric * n_dead_tup / nullif(n_live_tup, 0))::numeric(7,1) as dead_tup_pct
from q_tstats
...
select * from (
    select
        ...
        (100::numeric * sum(n_dead_tup)::int8 / sum(nullif(n_live_tup, 0))::int8)::numeric(7,1) as dead_tup_pct
      from
           q_tstats ts
